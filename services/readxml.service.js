    ...
    return new Promise(async (resolve, reject) => {
        request(url, { encoding: null }, (err, res, body) => {
            if (err) reject(err);
                ...
                const filteredXmlArr = xmlArr.filter(
                    (xmlElement) => xmlElement.slice(-6) === "</loc>"
                );
                const urlArr = filteredXmlArr.map((xmlElement) =>
                    xmlElement.slice(5, xmlElement.length - 6)
                );
                resolve(urlArr);
            }
        });
